...
The definition of this image is generated automatically based on other environments used in this project, so a new image can be generated by using the following scripts:

```
source .env; source env.sh
# Follow instructions at https://www.packer.io/docs/builders/googlecompute
...
# https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/user-data.html Explains why "user_data" can be #cloud-config cloud init file as yaml
# This should be "user-data" key of "metadata" https://cloud.google.com/container-optimized-os/docs/how-to/create-configure-instance
# You should be able to pass a file path according to https://blog.woohoosvcs.com/2019/11/cloud-init-on-google-compute-engine/

# From repo root:
conda activate cloudprovider
python scripts/cluster/cloudinit.py > config/dask/cloud-init-config.yaml

# See https://github.com/dask/dask-cloudprovider/blob/master/dask_cloudprovider/gcp/instances.py#L382
# See https://github.com/dask/dask-cloudprovider/blob/master/dask_cloudprovider/gcp/instances.py#L103
...
(cloudprovider) eczech@ukb-experiments:~/repos/ukb-gwas-pipeline-nealelab$ packer build config/dask/packer-config.json
...
--> googlecompute: A disk image was created: ukb-gwas-dask-1607612039
```
...
gcloud compute instances create test-image \
  ...
  --image ukb-gwas-dask-1607612039 \
  --zone $GCP_ZONE
```
...
python scripts/cluster/cloud-provider.py -- --interactive
...
# e.g. gcloud beta compute ssh --zone us-east1-c dask-f0bc0892-scheduler --ssh-flag="-L 8799:localhost:8787"
