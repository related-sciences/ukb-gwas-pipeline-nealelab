...
def load_genotype_ds(genotypes_path: str) -> Dataset:
    ...
    ds["call_genotype_probability_mask"] = ds["call_genotype_probability_mask"].astype(
        bool
    )
    return ds
...
def variant_qc(ds):
    ...
    ds["dummy"] = xr.DataArray(
        da.empty((ds.dims["variants"], ds.dims["samples"], 2)),
        dims=("variants", "samples", "ploidy"),
    )
    ds["variant_hwe_p_value"] = sgkit.hardy_weinberg_test(
        ds, genotype_counts="variant_genotype_counts"
    )["variant_hwe_p_value"]
